    ...
    for (double x = circle.x - circle.r; x <= circle.x + circle.r; x++){
        for (double y = circle.y - circle.r; y <= circle.y + circle.r; y++){
            ...
            if (distance_squared < radius_squared){
                ...
                SDL_Rect pixel = (SDL_Rect) {(int)round(x), (int)round(y), 1, 1};
...
void FillRays (SDL_Surface* surface, struct Ray rays [RAYS_NUMBER], Uint32 color, struct Circle object){
    ...
    for (int i = 0; i < RAYS_NUMBER; i++){
        ...
        while (!end_of_screen && !object_hit){
            ...
            SDL_Rect ray_point = (SDL_Rect) {(int)round(x_draw), (int)round(y_draw), RAY_THICKNESS, RAY_THICKNESS};
            double blur_size = 3 * RAY_THICKNESS;
            SDL_Rect ray_blur = (SDL_Rect) {(int)round(x_draw), (int)round(y_draw), (int)round(blur_size), (int)round(blur_size)};
